...
$(document).ready(function() {
	...
	// Create graph from the data table
	createGraph('#data-table', '.chart');
	
	// graphs
	function createGraph(data, container) {
		// variables and container elements	
		var bars = [];
		...
		function displayGraph(bars, i) {		
			...
			if (i < bars.length) {
				// how fast each bar moves individually
				$(bars[i].bar).animate({
					height: bars[i].height
				}, 1000);
				// how fast the next bar starts before the previous one
		...
		function resetGraph() {
			// animation's begining point
			$.each(bars, function(i) {
				$(bars[i].bar).stop().css('height', 1);
			});
